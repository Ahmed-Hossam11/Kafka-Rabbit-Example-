...
export class UsersService {
  ...
  async findById(filter:string): Promise<UserDocument> {
    console.log('here')
    const user = await this.userRepository.findOne({_id:filter});
    return user;
  }

  // updateDoc(item: TModel, options?: QueryOptions  )
  // {
  //   return this.model
  ...
  async updateTest(
    ...
    const user = await this.userRepository.updateOneVoid(filter, updateUserData);
