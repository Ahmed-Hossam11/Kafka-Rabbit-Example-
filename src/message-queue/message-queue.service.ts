import {
    ...
    implements OnModuleInit, OnApplicationShutdown
  {
    ...
    async onModuleInit(): Promise<void> { // start channel befor running modules 
      ...
      await new Promise<void>(async (resolve, _reject) => {
        ...
        this._operationsChannelWrapper = await this._connection.createChannel({
          setup: function (channel: Channel) {
            return Promise.all([
              channel.assertQueue(Constants.MessageQueues.TEST1, { //create channel between publisher and queue 
                durable: true,
              }),
            ]);
          },
        ...
        // console.log( this._connection.on('connect',))
